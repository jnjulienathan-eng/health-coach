...
_Last updated: July 11, 2026 (fix: PKCE code_challenge encoding mismatch in the OAuth 2.1 layer)_
...
**Known secondary issue found during this investigation, not yet fixed:** `GET /.well-known/oauth-protected-resource/api/mcp` 404s. Per RFC 9728, when a protected resource's identifier has a URL path (ours is `{origin}/api/mcp`), spec-compliant clients probe for metadata at `{origin}/.well-known/oauth-protected-resource/api/mcp` (well-known segment inserted before the resource's path), not just `{origin}/.well-known/oauth-protected-resource`. Our route only exists at the bare path. Observed directly via the real `mcp-remote` client's request log during this session; didn't block the flow (the client falls back to the bare path successfully), so left as a follow-up rather than folded into this fix.
