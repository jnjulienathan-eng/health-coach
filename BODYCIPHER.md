...
**RLS disabled.** Most reads/writes are still browser-direct via the anon key (`lib/db.ts`'s `saveEntry`/`loadEntry`/`loadAllEntries`/`loadRecentEntries`/`isSleepLogged`/`deriveCycleDay`/`getGoalsData`, all called from `app/page.tsx`) — not yet part of the RLS fix. As of Sept 7, 2026, the server-only rolling-average helpers (`getHrvRolling28DayMedian`, `getBedtimeRolling30DayAvg`, `getSleepDebtRolling7Day`) and `lib/scores-server.ts`/`app/api/coach/route.ts`'s own daily_entries reads/writes now run on the service-role client instead of anon — see "RLS security fix — status" under the RLS section below.
...
- **RLS disabled.** Browser-direct calls (`loadSessionsForDates`, bundled into `saveEntry`/`loadEntry`/`loadAllEntries`/`loadRecentEntries`) still use the anon key — not yet part of the RLS fix, coupled 1:1 to the same daily_entries call sites since sessions are always loaded/saved alongside the entry for a date. `loadSessionsForDates` itself gained an optional `client` param (Sept 7, 2026) so the two server-only callers (`lib/scores-server.ts`, `app/api/coach/route.ts`) can pass the service-role client — the default (used by every browser call site above) is unchanged. See "RLS security fix — status" under the RLS section below.
...
- RLS disabled — still 100% browser-direct (anon key), not yet part of the RLS fix. See "RLS security fix — status" under the RLS section below.
...
- No RLS mentioned in the migration. **RLS is still disabled as of Sept 7, 2026** — Julie will enable it manually in Supabase after confirming the route change below on the branch preview. Historically queried with the same anon-key `supabase` client as `daily_entries`/`training_sessions`/`health_appointments`, not the nutrition tables' service-role admin client — this is what the route change below fixes.
...
- **Route (added Sept 7, 2026):** `app/api/glp1/route.ts` — `GET` (list, ordered by date ascending) and `POST` (`{ date, dose_mg?, notes? }`, assigns `injection_number = max+1`), both via `supaAdmin()`. `app/page.tsx`'s `loadGlp1`/`handleLogGlp1` call this route via `fetch()` instead of talking to Supabase directly from the browser. `lib/db.ts`'s `loadGlp1Injections()`/`logGlp1Injection()` (the functions this route's logic was ported from) are left in place but unused, flagged with a comment — kept only until the route is confirmed working, do not re-wire them once RLS is on.
...
- Legacy tables (daily_entries, training_sessions, health_appointments, glp1_injections): user_id = 'julie' (text), **RLS disabled on all four as of Sept 7, 2026** — Supabase flags this as a critical "RLS Disabled in Public" warning. Do not enable RLS on any of these without also fixing every remaining anon-key call site for that table — see the multi-session fix below, tracked in the changelog at the top of this file.
- **RLS security fix — status as of Sept 7, 2026 (session 1 of a multi-session plan):**
  - ✅ Done: three server-only spots that were using the anon key despite running server-side now use the service-role client (`lib/scores-server.ts` recomputeScores, `app/api/scores/route.ts` GET, `app/api/coach/route.ts` getCoachContext + its direct rolling-average calls). No route changes, no behaviour change — pure client swap.
  - ✅ Done: `glp1_injections` moved off the browser-direct anon-key pattern — new `app/api/glp1/route.ts` (service-role), `app/page.tsx` now calls it via `fetch()`. **RLS on `glp1_injections` is NOT yet enabled** — pending Julie confirming the new route on the branch preview, then enabling it herself in the Supabase dashboard.
  - ⬜ Not done: `daily_entries` and `training_sessions` are still read/written directly from the browser via the anon `supabase` client in `lib/db.ts`, called from 11 sites across 7 functions in `app/page.tsx` (`loadEntry`, `saveEntry`, `loadAllEntries`, `loadRecentEntries`, `isSleepLogged`, `deriveCycleDay`, `getGoalsData`) — none of these can move behind a server route without a dedicated session, since there is currently no `/api/entries` route at all.
  - ⬜ Not done: `health_appointments` is still 100% browser-direct — 10 call sites across 4 functions (`getGoalsData`, `saveHealthAppointment`, `fetchHealthAppointments`, `seedDefaultAppointments`) in `app/page.tsx`, no server route exists for it yet.
  - RLS must not be enabled on daily_entries, training_sessions, or health_appointments until their respective browser-direct call sites above are moved behind server routes — doing so first would break the app (every one of those call sites would start getting rejected by Postgres, since the anon key carries no `auth.uid()` matching a `user_id = 'julie'` policy).
