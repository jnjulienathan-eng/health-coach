...
**CGM prep — glucose logging hook (added Sept 7, 2026, defensive only, no marker mapping yet):** `/api/health-import` now logs (metric name, units, and a 3-entry sample of raw data points — not the full payload) for any incoming metric whose name contains `"glucose"` (case-insensitive substring match, not an exact-name check) to the same debug-log path used for unrecognized metrics. Expected field name is `blood_glucose` — HAE's likely identifier for Apple's "Blood Glucose" HealthKit category, inferred from its naming pattern for other metrics (`heart_rate_variability`, `resting_heart_rate`) but **not yet confirmed against a real payload** — the substring match exists specifically in case the real name differs. No mapping to `cgm_readings` (or anywhere else) exists yet — no parsing, no writes. Next step once a real payload lands: confirm the field name and per-point shape from these logs, then wire an actual mapping in its own session. See DATA MODEL → `cgm_readings` above.
...
### `cgm_readings` (added Sept 7, 2026 — CGM integration prep, defensive only)
...
- **Table does not exist yet.** Migration below is written for Julie to run herself in the Supabase SQL editor — not run by Claude Code, per standing process.
  ```sql
  ...
  ```
- **No parsing/write logic exists yet** — see the Health Auto Export mapping note below. When it's built, any read/write must go through a server route on the service-role client (same pattern as `/api/entries`, `/api/glp1`, `/api/health-appointments`) — no browser-direct anon-key path, so this table never joins the RLS backlog that daily_entries/training_sessions/health_appointments just closed out.
