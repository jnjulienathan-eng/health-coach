...
- `weight_body_mass` → `biomarker_readings` with marker = `'weight'`, unit = `'kg'` (wired Aug 18, 2026). lb→kg converted if HAE sends units = `'lb'`; rounded to 1 decimal. **Overwrite-on-change**, not insert-only-if-absent like `vo2_max` above: the route selects the existing `(user_id, marker, recorded_on)` row first, and only upserts (`onConflict: 'user_id,marker,recorded_on'`, the same unique constraint `vo2_max`'s 23505 skip already relies on) when the incoming value differs from what's stored — same rule as `apple_hrv_avg` on `daily_entries`, adapted from a plain column compare to this table's composite-key compare. A resend with an unchanged value doesn't count toward `weightImported` (tracked separately from `metricsImported`/`vo2MaxImported`, same pattern as the other biomarker). **`weight_body_mass` is HAE's standard identifier for Apple's body-mass metric but was not verified against a real payload before wiring** — a deliberate one-attempt call, not a hardcoded-without-any-fallback guess: `app/api/health-import/route.ts` still logs `name` + `units` (not the full payload) for any metric it doesn't recognize, so if this name is wrong, weight silently won't populate and the real name will surface in logs on the next sync — fix is a one-line string change in `RECOGNIZED_METRIC_NAMES` and the `metric.name === 'weight_body_mass'` check. BMI is explicitly out of scope — no BMI mapping, storage, or display.
...
- markers in use: 'vo2_max', 'ldl', 'hdl', 'hba1c', 'weight' (unit 'kg' — webhook-written, overwrite-on-change, see HAE mapping note above), 'body_fat_pct' (unit '%'), 'waist_cm' (unit 'cm'), 'visceral_fat_l' (unit 'L')
