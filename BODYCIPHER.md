...
### `cgm_readings` (added Sept 7, 2026 — CGM integration prep; real write path added Sept 7, 2026, later same day)

One row per timestamped glucose reading — **not** a daily aggregate, unlike `daily_entries.fasting_glucose_mmol`. Sensor is Libre 3 Plus, syncing via LibreLink → GlucosePhone → Apple Health ("Blood Glucose") → Health Auto Export → `/api/health-import`.
...
- **No read UI/API route exists yet** — this is write-only. The Glucose Stability card (Today Tab → Long-term Goals) still shows its CGM-toggle placeholder (see Today Tab section) with no live data behind it. Building a read path (an `/api/cgm` route or similar, plus wiring the card) is a separate follow-up, not done this session.
