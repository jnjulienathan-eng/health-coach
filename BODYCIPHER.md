...
- RLS disabled. **Partially moved as of Sept 7, 2026 (session 2):** the health_appointments *read* inside `getGoalsData()` now goes through the service-role client via `app/api/goals` (session 2 moved `getGoalsData()` in its entirety, since it reads daily_entries/biomarker_readings/health_appointments together in one function — see the `daily_entries` entry above). The three dedicated health_appointments functions — `saveHealthAppointment`, `fetchHealthAppointments`, `seedDefaultAppointments` — are untouched and still 100% browser-direct (anon key); moving those is session 3's scope, and session 2 deliberately did not touch them. See "RLS security fix — status" under the RLS section below.
...
- **RLS security fix — status as of Sept 7, 2026 (session 2 of a multi-session plan):**
  ...
  - ✅ Partially done (session 2, as a side effect): `getGoalsData()` — which reads daily_entries, biomarker_readings, **and** health_appointments together in one function — moved onto the service-role client as part of the daily_entries work above, via new `app/api/goals/route.ts`. This reduces health_appointments' remaining scope for session 3 by one of its four functions/call sites.
  - ⬜ Not done: `health_appointments`'s three dedicated functions — `saveHealthAppointment`, `fetchHealthAppointments`, `seedDefaultAppointments` — are still 100% browser-direct anon-key calls from `app/page.tsx`. This is session 3's scope. Deliberately not touched this session per instruction.
  - RLS must not be enabled on daily_entries, training_sessions, or health_appointments until the testing above is done (for the first two) and health_appointments' remaining three functions are moved (session 3).
  - **Bugfix pass (same day, before merge):** live branch-preview testing surfaced a same-date Behavior Score inconsistency under rapid date-picker navigation — root-caused to a request-race in `loadDay()`/the score-loading effect (both `app/page.tsx`, fixed) and, separately, a pre-existing client-side score-recompute mismatch in the Dashboard's `HistoryRow` (unrelated to RLS or caching, predates both sessions, flagged but not fixed). Full writeup in the changelog entry at the top of this file. `export const dynamic = 'force-dynamic'` + `revalidate = 0` added to `/api/entries`, `/api/entries/sleep-logged`, `/api/entries/cycle-day`, `/api/goals`, `/api/scores`, and `/api/glp1`; `{ cache: 'no-store' }` added to every relevant client-side `fetch()` in `app/page.tsx`. This does not change the testing checklist above — still test everything listed before enabling RLS, and additionally confirm the Behavior/Outcome Score for a fixed date stays stable across several loads and rapid date-picker navigation.
